...
public class WavefrontObject {
	...
	{
		Log.i("OBJ", "Start loading OBJ: " + fileName);
		parserFactory = new ObjLineParserFactory(this);
		Log.i("OBJ", "Parser Factory created");
		//App;
		...
		InputStream fileInput = ResourceUtils.getInputStreamForRawFile(fileName);
		Log.i("OBJ", "InputStream got" + fileInput);
		//InputStream fileInput = this.getClass().getResourceAsStream(fileName);
		if (fileInput == null){
			// Could not find the file in the jar.
			Log.e("OBJ", "Couldn't open" + fileName);
			return;
		}
		
		
		BufferedReader in = null;
		Log.i("OBJ", "Start parsing OBJ: " + fileName);
		try 
		{
			in = new BufferedReader(new InputStreamReader(fileInput));
			Log.i("OBJ", "Created bufferedreader for OBJ: " + fileName + " " + in);
			...
			Log.e("OBJ", "Couldn't parse " + fileName + " " + e +" " + e.getMessage());
			throw new RuntimeException("Error reading file :'"+fileName+"'");
		}
		

		Log.i("OBJ", "Loaded OBJ from file '"+fileName+"'");
		Log.i("OBJ", getVertices().size()+" vertices.");
		Log.i("OBJ", getNormals().size()+" normals.");
		Log.i("OBJ", getTextures().size()+" textures coordinates.");
